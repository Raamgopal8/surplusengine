...
export default function AddSurplus({ setActiveTab }) {
  ...
  const handleSubmit = async () => {
    if (!user) {
      notify("Please sign in to list surplus items", "info");
      setActiveTab("login");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant font-label">Ingredient Type</label>
        <select 
      ...
      <div className="space-y-4">
        <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant font-label">Volume & Unit</label>
